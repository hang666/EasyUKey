...
type Device struct {
	...
	SerialNumber       string         `gorm:"not null;uniqueIndex:idx_device_serial" json:"serial_number"`        // 硬件序列号 (来自客户端)
	VolumeSerialNumber string         `gorm:"not null;uniqueIndex:idx_device_serial" json:"volume_serial_number"` // 卷序列号 (来自客户端)
